...
PhysioECG provides a complete pipeline for electrocardiogram (ECG)
analysis built on the PhysioExperiment data model. This vignette
demonstrates the core workflow: R-peak detection, RR interval
computation, ectopic beat handling, and heart rate variability (HRV)
...
PhysioECG includes data generators for creating synthetic ECG signals.
The
[`make_ecg()`](https://x-biosignal.github.io/PhysioECG/reference/make_ecg.md)
function produces a PhysioExperiment with Gaussian-shaped R-peaks at
regular intervals.
...
For testing with realistic noise, use
...
enables QRS-based SNR estimation for more accurate quality metrics.
...
function implements the Pan-Tompkins algorithm with adaptive
dual-threshold classification. It automatically handles inverted ECG
signals.
...
Standard frequency bands are VLF (0.003–0.04 Hz), LF (0.04–0.15 Hz), and
HF (0.15–0.4 Hz). Custom bands can be specified:
...
to compute all nonlinear metrics at once:
