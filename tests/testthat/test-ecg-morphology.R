...
# --- WS5-15: multi-formula QTc ---

test_that("QTc formulas coincide at RR=1s, order correctly, and match references", {
  qtc <- PhysioECG:::.ecgQTc
  qt <- 400  # ms

  # At RR = 1 s all four corrections equal QT
  r1 <- qtc(qt, 1.0)
  expect_equal(r1$bazett, qt, tolerance = 1e-6)
  expect_equal(r1$fridericia, qt, tolerance = 1e-6)
  expect_equal(r1$framingham, qt, tolerance = 1e-6)
  expect_equal(r1$hodges, qt, tolerance = 1e-6)

  # Tachycardia (RR < 1 s): Fridericia < Bazett
  r2 <- qtc(qt, 0.6)
  expect_lt(r2$fridericia, r2$bazett)

  # Independently hand-computed reference for QT = 360 ms, RR = 0.8 s
  # (literal constants so a units/constant regression actually fails)
  ref <- qtc(360, 0.8)
  expect_equal(ref$bazett,     402.4922, tolerance = 1e-3)
  expect_equal(ref$fridericia, 387.7982, tolerance = 1e-3)
  expect_equal(ref$framingham, 390.8000, tolerance = 1e-3)
  expect_equal(ref$hodges,     386.2500, tolerance = 1e-3)

  # Missing QT or RR -> NA for every formula
  expect_true(is.na(qtc(NA_real_, 0.8)$bazett))
  expect_true(is.na(qtc(360, NA_real_)$hodges))
})

test_that("ecgIntervals emits all four QTc columns, qtc_ms == qtc_bazett", {
  result <- make_ecg_pqrst(n_time = 10000, sr = 500, heart_rate = 72)
  pe <- result$pe
  ...
  for (col in c("qtc_bazett", "qtc_fridericia", "qtc_framingham", "qtc_hodges")) {
    expect_true(col %in% names(intervals))
  }
  expect_equal(intervals$qtc_ms, intervals$qtc_bazett)
})
