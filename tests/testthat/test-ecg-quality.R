...
# --- WS5-12: cubic-spline ectopic correction + burden ---

test_that("cubic_spline correction reproduces a linear RR ramp exactly", {
  n <- 20
  rr_true <- 700 + 10 * (seq_len(n) - 1)          # linear 700..890
  ect <- rep(FALSE, n); ect[c(6, 11, 16)] <- TRUE
  df <- data.frame(channel = 1L, rr_ms = rr_true, is_ectopic = ect)
  df$rr_ms[ect] <- 0                              # corrupt ectopic beats

  out <- ecgRRcorrect(df, method = "cubic_spline")
  expect_equal(out$rr_ms, rr_true, tolerance = 1e-6)   # exact on a line

  # linear interpolation agrees at the knots for a linear ramp
  out_lin <- ecgRRcorrect(df, method = "interpolate")
  expect_equal(out$rr_ms[ect], out_lin$rr_ms[ect], tolerance = 1e-6)
})

test_that("ectopic burden attributes and ecgEctopicBurden agree (~20%)", {
  set.seed(7)
  n <- 100
  ect <- (seq_len(n) %% 5 == 0)                   # every 5th beat -> 20%
  df <- data.frame(channel = 1L, rr_ms = 800 + rnorm(n, sd = 5),
                   is_ectopic = ect)

  out <- ecgRRcorrect(df, method = "cubic_spline")
  expect_equal(attr(out, "pct_ectopic"), 20, tolerance = 1e-9)
  expect_equal(attr(out, "n_ectopic"), 20)

  b <- ecgEctopicBurden(df)
  expect_equal(b$pct_ectopic, 20, tolerance = 1e-9)
  expect_equal(b$n_ectopic, 20L)
  expect_equal(b$n_total, 100L)
})

test_that("cubic-spline correction moves RMSSD toward the clean value", {
  set.seed(3)
  n <- 200
  clean <- 800 + rnorm(n, sd = 20)
  rmssd <- function(v) sqrt(mean(diff(v)^2))
  pos <- seq(10, n - 10, by = 15)
  ect <- rep(FALSE, n); ect[pos] <- TRUE
  corrupted <- clean; corrupted[pos] <- clean[pos] + 300   # ectopic spikes

  df <- data.frame(channel = 1L, rr_ms = corrupted, is_ectopic = ect)
  fixed <- ecgRRcorrect(df, method = "cubic_spline")$rr_ms

  d_before <- abs(rmssd(corrupted) - rmssd(clean))
  d_after  <- abs(rmssd(fixed)     - rmssd(clean))
