...
#' Create a two-belt RIP fixture
make_rip <- function(n_time = 600, sr = 10, f = 0.25,
                     a_rc = 1, a_ab = 1, phase_ab = 0) {
  time <- (seq_len(n_time) - 1) / sr
  rc <- a_rc * sin(2 * pi * f * time)
  ab <- a_ab * sin(2 * pi * f * time + phase_ab)
  PhysioExperiment(
    assays = list(raw = cbind(RC = rc, AB = ab)),
    colData = S4Vectors::DataFrame(
      label = c("RC", "AB"),
      type = c("RIP", "RIP")
    ),
