...
#'   uniformly resampled FFT-based PSD, \code{"lomb"} for Lomb-Scargle
#'   periodogram on unevenly sampled data, or \code{"ar"} for a Burg
#'   autoregressive PSD.
...
#'   (default: c(0.15, 0.4)).
#' @param detrend Logical; if \code{TRUE}, apply smoothness-priors detrending
#'   (\code{\link{smoothnessPriorsDetrend}}) to the resampled tachogram before
#'   spectral estimation (Welch method). Default \code{FALSE} (mean removal).
#' @param detrend_lambda Smoothing parameter passed to
#'   \code{\link{smoothnessPriorsDetrend}} when \code{detrend = TRUE}
#'   (default 500).
#' @param ar_order Integer AR model order for \code{method = "ar"}; \code{NULL}
#'   (default) selects the order automatically by AIC.
...
#'     \item{lf_nu, hf_nu}{LF and HF power in normalized units,
#'       \code{100 * LF/(LF+HF)} and \code{100 * HF/(LF+HF)}; these sum to 100.}
#'     \item{lf_peak, hf_peak}{Frequency (Hz) of the spectral peak within the
#'       LF and HF bands, or \code{NA} if the band is empty.}
#'   }
#'
...
ecgHRVfreq <- function(rr,
  ...
  for (ch in channels) {
    ...
    if (method == "welch") {
      psd_result <- .hrv_welch(t_sec, rr_ms, detrend = detrend,
                               lambda = detrend_lambda)
    } else if (method == "ar") {
      psd_result <- .hrv_ar_burg(t_sec, rr_ms, order = ar_order,
                                 detrend = detrend, lambda = detrend_lambda)
    } else {
    ...
    # Normalized units (LF/(LF+HF)) and within-band peak frequencies
    lf_hf <- lf + hf
    lf_nu <- if (lf_hf > 0) 100 * lf / lf_hf else NA_real_
    hf_nu <- if (lf_hf > 0) 100 * hf / lf_hf else NA_real_
    lf_peak <- .band_peak(freqs, psd, lf_band)
    hf_peak <- .band_peak(freqs, psd, hf_band)
...
#' @param detrend Logical; apply smoothness-priors detrending after resampling.
#' @param lambda Smoothing parameter for smoothness-priors detrending.
#' @return List with freqs and psd vectors.
#' @keywords internal
.hrv_welch <- function(t_sec, rr_ms, detrend = FALSE, lambda = 500) {
  ...
  # Detrend: smoothness-priors trend removal, or simple mean subtraction
  if (detrend) {
    rr_uniform <- smoothnessPriorsDetrend(rr_uniform, lambda = lambda, fs = fs)
  } else {
...
#' @param freqs Frequency vector (Hz).
#' @param psd Power spectral density vector.
#' @param band Numeric length-2 band (Hz).
#' @return Frequency (Hz) of maximum PSD within the band, or NA if empty.
#' @keywords internal
.band_peak <- function(freqs, psd, band) {
  in_band <- freqs >= band[1] & freqs <= band[2]
  if (!any(in_band)) return(NA_real_)
  fb <- freqs[in_band]
  fb[which.max(psd[in_band])]
}

#' Burg autoregressive PSD for HRV
#' @param t_sec Time stamps in seconds.
#' @param rr_ms RR intervals in milliseconds.
#' @param order AR model order, or NULL to select by AIC.
#' @param detrend Logical; apply smoothness-priors detrending after resampling.
#' @param lambda Smoothing parameter for smoothness-priors detrending.
#' @return List with freqs and psd vectors.
#' @keywords internal
.hrv_ar_burg <- function(t_sec, rr_ms, order = NULL, detrend = FALSE,
                         lambda = 500) {
  fs <- 4
  t_uniform <- seq(min(t_sec), max(t_sec), by = 1 / fs)
  x <- stats::approx(t_sec, rr_ms, xout = t_uniform)$y
  x <- x[is.finite(x)]
  if (detrend) {
    x <- smoothnessPriorsDetrend(x, lambda = lambda, fs = fs)
  } else {
    x <- x - mean(x)
  }

  n <- length(x)
  max_ord <- max(2L, min(if (is.null(order)) 30L else as.integer(order),
                         n %/% 2L - 1L))
  fit <- stats::ar.burg(x, order.max = max_ord, aic = is.null(order))
  a <- fit$ar
  sigma2 <- fit$var.pred
  ...
  for (i in seq_len(nf)) {
    w <- 2 * pi * freqs[i] / fs
    denom <- 1 - sum(a * exp(-1i * w * k))
    psd[i] <- sigma2 / (fs * Mod(denom)^2)
  }
  if (nf > 2) psd[2:(nf - 1)] <- 2 * psd[2:(nf - 1)]   # one-sided scaling
  list(freqs = freqs, psd = psd)
}
