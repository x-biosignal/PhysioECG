...
ecgQualityCheck <- function(rr, threshold_ms = 300) {
  ...
  for (ch in channels) {
    ...
    for (i in seq_len(n)) {
      ...
      local_med <- stats::median(rr_vals[win_idx], na.rm = TRUE)
      # A missing beat (or an all-NA window) cannot be classified; leave it as
      # non-ectopic rather than erroring on NA in the comparison.
      if (!is.na(rr_vals[i]) && !is.na(local_med) &&
          abs(rr_vals[i] - local_med) > threshold_ms) {
...
#' interpolation from surrounding non-ectopic intervals, \code{"cubic_spline"}
#' uses natural cubic-spline interpolation (C1-smooth, following Kubios-style
#' automatic correction), and \code{"remove"} simply drops ectopic rows.
#'
...
#'   ectopic values with linear interpolation; \code{"cubic_spline"} uses
#'   natural cubic-spline interpolation (falls back to linear when fewer than
#'   four valid beats are available in a channel); \code{"remove"} drops
#'   ectopic rows.
...
#'   as attributes \code{n_ectopic} (count) and \code{pct_ectopic} (percent of
...
#'
#' @export
ecgRRcorrect <- function(rr, method = c("interpolate", "cubic_spline", "remove")) {
  stopifnot(is.data.frame(rr))
  ...
  n_total <- nrow(rr)
  n_ectopic <- sum(rr$is_ectopic)
  pct_ectopic <- if (n_total > 0) 100 * n_ectopic / n_total else NA_real_

  if (method == "remove") {
    out <- rr[!rr$is_ectopic, , drop = FALSE]
    out$is_ectopic <- NULL
    rownames(out) <- NULL
    attr(out, "n_ectopic") <- n_ectopic
    attr(out, "pct_ectopic") <- pct_ectopic
    return(out)
  }

  # Interpolation methods (linear or natural cubic spline)
  channels <- unique(rr$channel)
  ...
  for (ch in channels) {
    ...
    if (method == "cubic_spline" && length(valid) >= 4) {
      interpolated <- stats::spline(
        x = valid, y = rr_vals[valid],
        xout = invalid, method = "natural"
      )$y
  ...
  rr$is_ectopic <- NULL
  attr(rr, "n_ectopic") <- n_ectopic
  attr(rr, "pct_ectopic") <- pct_ectopic
  rr
}

#' Ectopic Beat Burden
#'
#' Convenience summary of the ectopic-beat burden in an RR-interval data.frame
#' that has been passed through \code{\link{ecgQualityCheck}}.
#'
#' @param rr A data.frame with an \code{is_ectopic} logical column.
#' @return A list with \code{n_ectopic} (count of ectopic beats),
#'   \code{n_total} (total beats) and \code{pct_ectopic} (percent ectopic).
#' @seealso \code{\link{ecgQualityCheck}}, \code{\link{ecgRRcorrect}}
#' @export
#' @examples
#' rr <- data.frame(channel = 1L, rr_ms = c(800, 810, 400, 805, 815),
#'                  is_ectopic = c(FALSE, FALSE, TRUE, FALSE, FALSE))
#' ecgEctopicBurden(rr)
ecgEctopicBurden <- function(rr) {
  stopifnot(is.data.frame(rr))
  if (!"is_ectopic" %in% names(rr)) {
    stop("Missing required column: is_ectopic. Run ecgQualityCheck first.")
  }
