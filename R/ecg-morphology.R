...
#'       unavailable. Kept as a backward-compatible alias of
#'       \code{qtc_bazett}.}
#'     \item{qtc_bazett, qtc_fridericia, qtc_framingham, qtc_hodges}{QT
#'       corrected for heart rate by, respectively, Bazett
#'       (\code{QT / sqrt(RR)}), Fridericia (\code{QT / RR^(1/3)}),
#'       Framingham (\code{QT + 154 * (1 - RR)}) and Hodges
#'       (\code{QT + 1.75 * (HR - 60)}); QT in ms, RR in seconds. All four
#'       equal \code{qt_ms} at \code{RR = 1 s}.}
...
ecgIntervals <- function(delineation, sr) {
  ...
  for (ch in channels) {
    ...
    for (i in seq_len(n_beats)) {
      ...
      # QTc via multiple rate-correction formulas (QT in ms, RR in seconds)
      qtc <- .ecgQTc(qt_ms, if (is.na(rr_ms)) NA_real_ else rr_ms / 1000)
      ...
      results[[length(results) + 1]] <- data.frame(
        ...
        pr_ms = pr_ms,
        qt_ms = qt_ms,
        qtc_ms = qtc$bazett,             # backward-compatible Bazett alias
        qtc_bazett = qtc$bazett,
        qtc_fridericia = qtc$fridericia,
        qtc_framingham = qtc$framingham,
        qtc_hodges = qtc$hodges,
        qrs_ms = qrs_ms,
        rr_ms = rr_ms,
        stringsAsFactors = FALSE
      )
  ...
  if (length(results) > 0) {
    ...
    data.frame(
      ...
      qtc_bazett = numeric(0), qtc_fridericia = numeric(0),
      qtc_framingham = numeric(0), qtc_hodges = numeric(0),
      qrs_ms = numeric(0), rr_ms = numeric(0)
    )
  }
}

# Rate-corrected QT via four standard formulas. qt_ms in ms, rr_sec in seconds.
# All four equal qt_ms exactly at rr_sec == 1 s. Returns NA when QT/RR missing.
.ecgQTc <- function(qt_ms, rr_sec) {
  if (is.na(qt_ms) || is.na(rr_sec) || rr_sec <= 0) {
    return(list(bazett = NA_real_, fridericia = NA_real_,
                framingham = NA_real_, hodges = NA_real_))
  }
  hr <- 60 / rr_sec
  list(
    bazett     = qt_ms / sqrt(rr_sec),
